...
const BASIC_EMAIL_PATTERN = /^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$/;
...
function getAllowedEmailDomain() {
  return String(process.env.ALLOWED_EMAIL_DOMAIN || "")
    .trim()
    .toLowerCase()
    .replace(/^@+/, "");
}
...
function isAllowedCollegeEmail(email) {
  ...
  if (!allowedDomain || !BASIC_EMAIL_PATTERN.test(normalizedEmail)) {
    return false;
  }
  ...
  return Boolean(localPart) && domain === allowedDomain;
}
